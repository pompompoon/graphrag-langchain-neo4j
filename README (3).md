...
ローカルLLMと知識グラフを組み合わせたRAG（Retrieval-Augmented Generation）チャットアプリケーション。テキストからエンティティを自動抽出し、知識グラフを構築・可視化・検索する。
...
graphrag-app/
├── backend/
│   ├── .env                    # 環境設定（PROVIDER, STORAGE, APIキー等）
│   ├── config.py               # LLM/Embeddings/ストレージの一元管理
│   ├── main.py                 # FastAPI エンドポイント
│   ├── graph_agent.py          # LangGraph エージェント（4ノード構成）
│   ├── entity_extractor.py     # LLMエンティティ抽出（LangChain Chain）
│   ├── neo4j_store.py          # Neo4jストレージバックエンド
│   ├── graph_rag.py            # ファイルベースストレージ（従来版）
│   ├── requirements.txt
...
Neo4j Desktop（https://neo4j.com/download/）をインストールし、新規DBを作成。
...
config/httpd-graphrag.conf を Apache の conf/extra/ にコピーし、httpd.conf に以下を追加：

```apache
Include conf/extra/httpd-graphrag.conf
```

必要なモジュール：mod_proxy, mod_proxy_http, mod_rewrite, mod_headers
...
| http://localhost:7474 | Neo4j Browser（Neo4jモード時） |

## 使い方

### ドキュメント追加

サイドバーのテキストエリアにテキストを貼り付けて「グラフに追加」。自動でエンティティ抽出、グラフ構築、ベクトルインデックス追加が行われる。

### グラフ可視化

ヘッダーの「グラフ」タブに切り替え。ノードドラッグ、ホイールズーム、背景パンに対応。右上の検索バーからベクトル検索でエンティティをハイライト表示。

### チャット

ヘッダーの「チャット」タブで質問を入力。LangGraphエージェントが検索→生成→品質チェック（スコア0.6未満なら最大2回リトライ）のフローで回答。

## .env 設定リファレンス

```bash
# LLMプロバイダ（コード変更不要で切り替え可能）
PROVIDER=ollama          # ollama / gemini

# ストレージ（コード変更不要で切り替え可能）
STORAGE=neo4j            # file / neo4j
...
全コンポーネントが config.py 経由でLLM/Embeddingsを取得する。プロバイダ固有のコードは config.py 内に閉じており、他のファイルは一切変更不要。

```
config.py ──→ get_chat_llm()     → graph_agent.py（回答生成・品質チェック）
           ──→ get_extract_llm() → entity_extractor.py（エンティティ抽出）
           ──→ get_embeddings()  → graph_rag.py / neo4j_store.py（ベクトル検索）
```

将来 OpenAI / Anthropic / Azure 等に切り替える場合も、config.py に分岐を追加するだけで対応可能。
...
Neo4j Browserで直接Cypherクエリを実行できる：
...
-- ベクトル類似検索（Cypherから直接）
CALL db.index.vector.queryNodes('entity_embedding_index', 5, $queryVector)
YIELD node, score
...
| GET | /api/graph/data?query= | グラフ可視化データ（検索ハイライト付き） |
...
### hnswlib インストールエラー
pip install chroma-hnswlib でプリビルド版を使用。

### uvicorn が見つからない
hnswlibのエラーで pip install が中断している。先にhnswlib問題を解決してから再実行。

### npm が見つからない
conda install nodejs -y で Node.js をインストール。

### GraphView.js の大文字小文字エラー
Windows でファイル名を2段階でリネーム：ren GraphView.js _temp.js → ren _temp.js GraphView.js

### eslint react-hooks/exhaustive-deps エラー
GraphView.js から該当コメント行を手動で削除。PowerShellの Set-Content はエンコーディングを変えるため、テキストエディタでの編集を推奨。

### グラフデータの取得に失敗
ファイルモード: backend/data/ を削除してuvicorn再起動。
Neo4jモード: Neo4jが起動しているか確認。http://localhost:7474 でNeo4j Browserにアクセスできるか確認。

### Neo4j ベクトルインデックスエラー
Neo4j 5.11以上が必要。Neo4j Desktopでバージョンを確認。
